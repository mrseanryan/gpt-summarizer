...
parser = OptionParser(
    usage='%prog <path to input file or input directory> [options]')
parser.add_option('-l', '--language', dest='target_language', default=config.TARGET_LANGUAGE,
...
def _get_path_to_output_file(path_to_input_file, path_to_output_dir):
    ...
    output_filename = util_file_ext.change_extension(input_filename, ".yaml.txt")  # adding .txt so can preview in Windows Explorer, Dropbox etc.
...
def summarize_one_file(path_to_input_file, target_language, path_to_output_dir):
    ...
    util_print.print_result(f" -- THIS FILE time: {util_time.describe_elapsed_seconds(elapsed_seconds)}")
    if cost > 0:
        util_print.print_important(f" -- THIS FILE estimated cost: {config.OPENAI_COST_CURRENCY}{cost}")

    path_to_output_file = _get_path_to_output_file(path_to_input_file, path_to_output_dir)
...
if os.path.isdir(path_to_input_file_or_dir):
    for extension in config.SUPPORTED_FILE_EXTENSIONS:
        input_filepaths += util_dir.find_files_recursively(path_to_input_file_or_dir, extension)
else:
...
for path_to_input_file in input_filepaths:
    ...
    if path_to_output_file and os.path.exists(path_to_output_file):
        util_print.print_warning(f"[skipping] output file '{path_to_output_file}' already exists")
        files_skipped += 1
        continue

    (_elapsed_seconds, _cost) = summarize_one_file(path_to_input_file, target_language, path_to_output_dir)
    elapsed_seconds += _elapsed_seconds
    cost += _cost
    files_processed += 1

util_print.print_section("Completed")
util_print.print_result(f"{files_processed} files processed in {util_time.describe_elapsed_seconds(elapsed_seconds)}")
if files_skipped > 0:
